...
class RetryDecorator(object):
    ...
    def __call__(self, f):
        ...
        def func(*args, **kwargs):
            ...
            try:
                result = done.wait()
                LOG.debug(_("Function %s returned successfully."), f.__name__)
            finally:
                loop.stop()
            return result
...
class VMwareAPISession(object):
    ...
    def wait_for_task(self, task):
        ...
        try:
            task_info = done.wait()
            LOG.debug(_("Task: %s completed successfully."), task)
        finally:
            loop.stop()
        return task_info
    ...
    def _poll_task(self, task, done):
        ...
        :param task: managed object reference of the task
        :param done: event capturing the task result or any exception
        """
    ...
    def _poll_lease(self, lease, done):
        ...
        :param done: event to notify completion of the poll
        """
        ...
        try:
            ...
            if state == 'ready':
                ...
                LOG.debug(_("Invoking VIM API to read lease: %s error."),
                          lease)
                error_msg = self.invoke_api(vim_util,
                                            'get_object_property',
                                            self.vim,
                                            lease,
                                            'error')
                excep_msg = _("Lease: %(lease)s is in error state. Details: "
                              "%(error_msg)s.") % {'lease': lease,
                                                   'error_msg': error_msg}
                LOG.error(excep_msg)
                done.send_exception(exceptions.VimException(excep_msg))
            else:
                # unknown state
