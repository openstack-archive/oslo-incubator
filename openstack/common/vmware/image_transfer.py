...
class BlockingQueue(queue.LightQueue):
    ...
    def read(self, chunk_size):
        ...
        if self._transferred < self._max_transfer_size:
            ...
            LOG.debug(_("Read %(bytes)d items out of %(max)d from queue."),
                      {'bytes': self._transferred,
                       'max': self._max_transfer_size})
            return data_item
        else:
...
def _start_transfer(context, timeout_secs, read_file_handle, max_data_size,
    ...
    LOG.debug(_("Starting image transfer with reader: %(reader) and writer: "
                "%(writer)s"),
              {'reader': reader,
               'writer': writer})
    reader.start()
...
def upload_image(context, timeout_secs, image_service, image_id, owner_id,
    ...
    # Otherwise, the image service client will use the volume size which
    # may not be the image size after upload, since it is converted to a
    # stream-optimized sparse disk.
