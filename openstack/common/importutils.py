...
class LazyPluggable(object):
    ...
    1. Registration of option in config::
        ...
        test_backend_opt = cfg.StrOpt('test_backend',
                                      default='foo',
                                      help='bar')
        CONF = cfg.CONF
        CONF.register_opt(test_backend_opt)

    2. Creating IMPL - object of class LazyPluggable::

        IMPL = utils.LazyPluggable('test_backend',
                                   CONF,
                                   foo=path_to_default,
                                   account_identifier
                                   =path_to_account_identifier)
    """
        ...
        if not self._backend:
            ...
            if isinstance(backend, tuple):
                if len(backend) == 2:
                    ...
                    msg = _('Length of backend tuple is not equal to 2')
