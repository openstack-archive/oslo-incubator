...
class ConfigOpts(collections.Mapping):
    ...
    def __call__(self, args=None):
        ...
        If the --config-dir option is set, all *.conf files from this
        directory are pulled in, as opposed to the file(s) specified by
        ...
        def _list_config_dir():
            files = glob.glob(os.path.join(self.config_dir, '*.conf'))
            files.sort()
            return files

        self._parse_config_files(_list_config_dir() if self.config_dir
