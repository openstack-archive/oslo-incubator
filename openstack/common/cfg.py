...
class ConfigOpts(collections.Mapping):
    ...
    def find_file(self, name):
        ...
        The config_file option is iterated over and each of the base
        directories of the config_files values are searched. Then the
        directory, if any, supplied by the config_dir option is searched.
        Failing both of these, the standard directories searched by the
        module level find_config_files() function is used. The first
        matching file is returned.

        :param basename: the filename, e.g. 'policy.json'
        :returns: the path to a matching file, or None
        """
        dirs = [os.path.dirname(cf) for cf in reversed(self.config_file)]
        dirs += [self.config_dir] if self.config_dir else []
        dirs += _get_config_dirs(self.project)
