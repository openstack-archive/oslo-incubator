        ...
        if __USE_ARGPARSE__:
            if action == 'store_true':
                return super(BoolOpt,
                             self)._get_optparse_kwargs(group, action=BoolOpt._StoreTrueAction, **kwargs)
            else:
                return super(BoolOpt,
                             self)._get_optparse_kwargs(group, action=BoolOpt._StoreFalseAction, **kwargs)
        else:
            return super(BoolOpt,
    ...
    if __USE_ARGPARSE__:
        class _OptionGroup(argparse._ArgumentGroup):
            ...
            def __init__(self, container, title=None, description=None, **kwargs):
                argparse._ArgumentGroup.__init__(self, container, title, description, **kwargs)
    ...
    def _get_optparse_group(self, parser):
        if self._optparse_group is None:
            if __USE_ARGPARSE__:
                ...
                #self._optparse_group = self._OptionGroup(self.title, self.help)
                self._optparse_group = parser._parser.add_argument_group(self.title,
                                                                        self.help)
            else:
                """Build an optparse.OptionGroup for this group."""
                #self._optparse_group = self._OptionGroup(parser._parser,
                #                                            self.title,
                #                                            self.help)
    ...
    if __USE_ARGPARSE__:
        ...
        class _OptionParser(argparse.ArgumentParser):
            ...
            def __init__(self, prog = None, usage = None, version = None, *args, **kwargs):
                argparse.ArgumentParser.__init__(self,usage = usage, prog = prog, *args, **kwargs)
                if usage is not None:
                    self.usage = usage.replace("%prog", self.prog)
                self.add_option('--version', action=ConfigParserCli._VersionAction, version = version)
                self._optionals.title = 'Options'

            def parse_args(self, *args, **kwargs):
                opts, args = argparse.ArgumentParser.parse_known_args(self, *args, **kwargs)
                #opts, args = argparse.ArgumentParser.parse_args(self, *args, **kwargs)
                unknown_args = []
                for arg in args:
                    if 0 == arg.find("-"):
            ...
            def print_version(self, file=None):
                argparse.ArgumentParser._print_message(self.format_version(), file)
