...
class BaseMigrationTestCase(test.BaseTestCase):
    ...
    # sqlalchemy.ext.declarative. Here inheriting classes should specify
    # this base class.
    MODEL_BASE = None
    DIALECT = None
    ...
    def _get_engine(self):
        connect_string = _get_connect_string(self.DIALECT,
                                             self.USER,
                                             self.PASSWORD,
                                             self.DATABASE)
        engine = sqlalchemy.create_engine(connect_string)
        self.engines[self.DIALECT] = engine
        self.test_databases[self.DIALECT] = connect_string
        return engine
...
class WalkVersionsMixin(object):
    ...
    @patch('openstack.common.db.sqlalchemy.session.get_engine')
    @patch('openstack.common.db.sqlalchemy.session.get_session')
    def _alembic_command(self, alembic_command, engine, *args, **kwargs):
        ...
        LOG.debug(_('Alembic command `%(command)s` returns: %(ret)s'),
                  {'command': alembic_command, 'ret': res})
        return res
    ...
    def _alembic_dbsync(self, engine):
        self._alembic_command('upgrade', engine, self.ALEMBIC_CONFIG, "head")

    def _dbsync(self, engine):
        """Helper method that sync database with presented repo.
        """
        if self._migrate_available():
            self._migrate_dbsync(engine)
        if self._alembic_available():
            self._alembic_dbsync(engine)

    def _migrate_available(self):
        return self.MIGRATION_API is not None and self.REPOSITORY is not None

    def _alembic_available(self):
...
class SyncModelsWithMigrations(BaseMigrationTestCase, WalkVersionsMixin):
    ...
    EXCLUDE_TABLES = ['migrate_version', 'alembic_version']

    def check_dialect(func):
        @functools.wraps(func)
        def _inner(self):
            if self.MODEL_BASE is None:
                self.skipTest("Base model is not declared")
            if self.DIALECT == 'mysql':
                check_backend = _have_mysql
            elif self.DIALECT == 'postgres':
                check_backend = _have_postgresql
            else:
                self.skipTest("%s is not supported" % self.DIALECT)
            if not check_backend(self.USER, self.PASSWORD, self.DATABASE):
                self.skipTest("%s not available" % self.DIALECT)
            func(self)
        return _inner
    ...
    def test_sync_models(self):
        ...
        tables = self._check_intersection(db_tables.keys(),
                                          models_tables.keys(),
                                          "Tables",
                                          "models",
        ...
        for table_name in tables:
            ...
            uc = self._check_uniques(table, db_tables[table_name])
