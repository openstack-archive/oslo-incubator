...
class ModelsMigrationsSync(object):
    ...
    @staticmethod
    def compare_server_default(cntx, ins_col, meta_col,
                               insp_def, meta_def, rend_meta_def):
        ...
        BoolType = sqlalchemy.Boolean
        dialect = cntx.dialect.name
        if dialect == 'mysql' and isinstance(meta_def.arg.type, BoolType):
            TrueType = sqlalchemy.sql.True_
            FalseType = sqlalchemy.sql.False_
            def_cast = int(insp_def.strip("'\""))
            return not (isinstance(meta_def.arg, TrueType) and def_cast == 1 or
                        isinstance(meta_def.arg, FalseType) and def_cast == 0)
