    ...
    def _reset_databases(self, tear_down=False):
        for key, engine in self.engines.items():
            ...
            if conn_string.startswith('sqlite'):
                ...
                continue
            dialect = 'mysql' if conn_string.startswith('mysql') else \
                      'postgres'
...
class WalkVersionsMixin(object):
    ...
    def _alembic_command(self, alembic_command, *args, **kwargs):
        ...
        self.ALEMBIC_CONFIG.print_stdout = \
            lambda rev: ret.append(str(rev).split("\n")[0])
        getattr(command, alembic_command)(*args, **kwargs)
        del self.ALEMBIC_CONFIG.print_stdout
        LOG.debug('Alembic command `%s` returns: %s' % (alembic_command,
                                                        "\n".join(ret)))
        return ret
    ...
    def _get_alembic_versions(self):
        ...
        """
        ret = []
        alembic_history = self._alembic_command('history', self.ALEMBIC_CONFIG)
...
class SyncModelsWithMigrations(BaseMigrationTestCase, WalkVersionsMixin):
    ...
    def check_dialect(func):
        @wraps(func)
        def _inner(self, dialect, engine):
            ...
            self._create_database(dialect, self.USER, self.PASSWD, database)
            if not check_backend(self.USER,
                                 self.PASSWD,
                                 database):
                self._drop_database(dialect, self.USER, self.PASSWD, database)
    ...
    def _check_column_type(self, column_model, column_db, column_name,
        ...
        if isinstance(column_model.type,
            ...
            if not isinstance(column_db.type, type(model_type)) and \
               (type(model_type), type(column_db.type)) not in CORRECT_TYPES:
                params['model'] += ':' + type(model_type).__name__
