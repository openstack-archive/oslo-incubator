...
def get_engine():
    ...
    global _ENGINE
    if _ENGINE is None:
        _ENGINE = create_engine(CONF.DATABASE.sql_connection)
    return _ENGINE
...
def create_engine(sql_connection):
    ...
    if CONF.DATABASE.sql_connection_debug >= 100:
        engine_args['echo'] = 'debug'
    elif CONF.DATABASE.sql_connection_debug >= 50:
        engine_args['echo'] = True

    if "sqlite" in connection_dict.drivername:
        engine_args["poolclass"] = NullPool

        if CONF.DATABASE.sql_connection == "sqlite://":
            engine_args["poolclass"] = StaticPool
            engine_args["connect_args"] = {'check_same_thread': False}
    else:
        engine_args['pool_size'] = CONF.DATABASE.sql_max_pool_size
        if CONF.DATABASE.sql_max_overflow is not None:
            engine_args['max_overflow'] = CONF.DATABASE.sql_max_overflow
    ...
    if 'mysql' in connection_dict.drivername:
        ...
        if not CONF.DATABASE.sqlite_synchronous:
            sqlalchemy.event.listen(engine, 'connect',
                                    _synchronous_switch_listener)
        sqlalchemy.event.listen(engine, 'connect', _add_regexp_listener)

    if (CONF.DATABASE.sql_connection_trace and
    ...
    try:
        ...
        remaining = CONF.DATABASE.sql_max_retries
        if remaining == -1:
            remaining = 'infinite'
        while True:
            ...
            if remaining != 'infinite':
                remaining -= 1
            time.sleep(CONF.DATABASE.sql_retry_interval)
            try:
                engine.connect()
                break
            except sqla_exc.OperationalError as e:
