...
def _zxcv(conn, user, pwd):
    front, back = conn.split('://')
    return '%s://%s:%s@%s' % (front, user, pwd, back)
...
def get_engine(sqlite_fk=False, slave_engine=False):
    ...
    engine = _ENGINE
    db_conn, db_usr, db_pwd = _asdf(CONF.database.connection)
    ...
    db_uri = _zxcv(CONF.database.connection,
                   CONF.database.username,
                   CONF.database.password)

    if slave_engine:
        engine = _SLAVE_ENGINE
        db_conn, db_usr, db_pwd = _asdf(CONF.database.slave_connection)
        ...
        db_uri = _zxcv(CONF.database.slave_connection,
