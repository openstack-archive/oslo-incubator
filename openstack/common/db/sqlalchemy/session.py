...
def _set_session_sql_mode(dbapi_con, connection_rec,
    ...
    result = dbapi_con.cursor().execute("SHOW VARIABLES LIKE 'sql_mode';")
    try:
        realmode = result.first()['Value']
        msg = _('MySQL server mode set to %s') % realmode
        if realmode.upper() != sql_mode.upper():
            msg += _(' (by server)')
        LOG.info(msg)
        if realmode.upper() not in ['TRADITIONAL', 'STRICT_ALL_TABLES']:
            LOG.warn(_("MySQL SQL mode is '%s', "
                       "consider setting to TRADITIONAL or STRICT_ALL_TABLES")
                     % realmode)
    except sqla_exc.NoSuchColumnError:
