...
def _execute_sql(engine, sql, driver):
    ...
    try:
        ...
        msg = ('%s does not match database admin credentials or database does'
               ' not exist. Please check env variable'
               ' OS_TEST_DBAPI_ADMIN_CONNECTION.')
        raise sqlalchemy.exc.ResourceClosedError(msg % SQL_CONNECTION)
