...
def _execute_sql(engine, sql, driver):
    ...
    try:
        ...
        msg = ('Env variable OS_TEST_DBAPI_ADMIN_CONNECTION does not '
               'match database admin credentials or database does not exist.')
        raise sqlalchemy.exc.ResourceClosedError(msg)
...
def main():
    ...
    parser.add_argument(
        '--tmp_dbs',
        action='store_true',
        help='Top level command to manpulate temporary test databases.')
