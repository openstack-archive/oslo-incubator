...
class AlembicExtension(ext_base.MigrationExtensionBase):
    ...
    def revision(self, message=None, autogenerate=False, sql=False,
        ...
        if autogenerate:
            ...
            def retrieve_migrations(rev, context):
                if (script_dir.get_version(rev)
                        is not script_dir.get_version("head")):
        ...
        with self.engine.connect() as conn:
            if environ:
                with environment.EnvironmentContext(
                    ...
                    env.configure(connection=conn)
                    script_dir.run_env()
        script_dir.generate_version(util.rev_id(), message, **template_args)
    ...
    def stamp(self, version, sql=False, tag=None):
        ...
        def do_stamp(rev, context):
            ...
            dest = script_dir.get_version(version)
            if dest is not None:
                dest = dest.version
