...
def _read_deleted_filter(query, db_model, read_deleted):
    if not read_deleted and 'deleted' not in db_model.__table__.columns:
        raise ValueError(_("There is no `deleted` column in `%s` table. "
                           "Project doesent use soft-deleted feature.")
                         % db_model.__name__)
...
def _project_filter(query, db_model, context, project_only):
    if not project_only and 'project_id' not in db_model.__table__.columns:
...
def model_query(context, model, args=None, project_only=False,
    ...
    if not read_deleted:
        if hasattr(context, 'read_deleted'):
            # NOOTE(viktors): some projects use `read_deleted` attribute in
    ...
    if not issubclass(model, models.ModelBase):
        raise ValueError(_("db_model should be subclass of ModelBase"))
