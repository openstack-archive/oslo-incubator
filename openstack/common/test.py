...
class ModelsObjectComparatorMixin(object):
    def _dict_from_object(self, obj, ignored_keys):
        if ignored_keys is None:
            ignored_keys = []
        return dict(
            [(k, v) for k, v in six.iteritems(obj) if k not in ignored_keys]
        )
    ...
    def assertEqualListsOfObjects(self, objs1, objs2, ignored_keys=None):
        ...
        :param objs1: First set of object. An 'object' is an instance of
            ModelBase
        :param objs2: Second set of objects
        :param ignored_keys: Set of keys that will be ignored, when
            comparing
        :type ignored_keys: set or None
        """
