...
class ModelsObjectComparatorMixin(object):
    def _dict_from_object(self, obj, ignored_keys):
        if ignored_keys is None:
            ignored_keys = []
        return dict(
            [(k, v) for k, v in obj.iteritems() if k not in ignored_keys]
        )

    def assertEqualObjects(self, obj1, obj2, ignored_keys=None):
        """Check equal of the two objects

        Object is model instances

        Keyword Arguments:
        obj1 -- first object
        obj2 -- second objects
        ignored_keys -- set of keys that will be ignored, when comparing
    ...
    def assertEqualListsOfObjects(self, objs1, objs2, ignored_keys=None):
        ...
        Keyword Arguments:
        objs1 -- first set of object
        objs2 -- second set of objects
        ignored_keys -- set of keys that will be ignored, when comparing
    ...
    def assertEqualListsOfPrimitivesAsSets(self, primitives1, primitives2):
        """Check equal of the two sets.

        Keyword Arguments:
        primitives1 -- first set
        primitives2 -- second set
