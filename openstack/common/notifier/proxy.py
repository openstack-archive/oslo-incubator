...
class NotifierProxy(object):
    ...
    def audit(self, ctxt, event_type, payload):
        ...
        api.notify(ctxt, self.publisher_id, event_type, payload, 'INFO')

    def debug(self, ctxt, event_type, payload):
        api.notify(ctxt, self.publisher_id, event_type, payload, 'DEBUG')

    def info(self, ctxt, event_type, payload):
        api.notify(ctxt, self.publisher_id, event_type, payload, 'INFO')

    def warn(self, ctxt, event_type, payload):
        api.notify(ctxt, self.publisher_id, event_type, payload, 'WARN')

    warning = warn

    def error(self, ctxt, event_type, payload):
        api.notify(ctxt, self.publisher_id, event_type, payload, 'ERROR')

    def critical(self, ctxt, event_type, payload):
        api.notify(ctxt, self.publisher_id, event_type, payload, 'CRITICAL')


def get_notifier(service=None, host=None, publisher_id=None):
    if not publisher_id:
        publisher_id = "%s.%s" % (service, host or CONF.host)
    return NotifierProxy(publisher_id)
