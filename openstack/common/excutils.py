    ...
    def __call__(self, f):
        ...
        def _func(*args, **kwargs):
            ...
            try:
                ...
                with save_and_reraise_exception() as ctxt:
                    logging.warn(_LW("Exception which is in the suggested "
                                     "list of exceptions occurred while "
                                     "invoking function: %s."),
                                 func_name, exc_info=True)
                    if (self._max_retry_count != -1 and
                            self._retry_count >= self._max_retry_count):
                        logging.error(_LE("Cannot retry upon suggested "
                                          "exception since retry count "
                ...
                with save_and_reraise_exception():
                    logging.exception(_LE("Exception which is not in the "
                                          "suggested list of exceptions "
                                          "occurred while invoking %s."),
                                      func_name)
