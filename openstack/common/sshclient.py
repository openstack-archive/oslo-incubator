...
class SSH(object):
    ...
    def _expanduser(self, path):
        ...
        if self.user == 'root':
            ...
            prefix = '/home'
        return os.path.join(prefix, self.user, path[2:])
    ...
    def upload(self, source, destination):
        ...
        # NOTE: This is still not eventlet compliant.
        #       It can be repleced by something like this:
        #       self.run('cat > %(dst)s', stdin=open(src, 'rb'))
        destination = self._expanduser(destination)
        client = self._get_client()
    ...
    def download(self, source, destination):
        ...
        source = self._expanduser(source)
        client = self._get_client()
        ftp = client.open_sftp()
        ...
        while True:
            try:
                ...
                LOG.debug(_('Ssh is still unavailable.'
                            ' Last exception was: %r') % e)
                eventlet.sleep(interval)
            if (time.time() - timeout) > start_time:
                raise SSHTimeout(_('Timeout waiting for ssh '
                                   'host "%s"') % self.host)
