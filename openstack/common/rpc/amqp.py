...
class Pool(pools.Pool):
    ...
    def empty(self):
        ...
        # Note that these lines were added due to the unit test cases failing
        # without them. The issue is the above while loop gets all the cached
        # connections from the pool and closes them, but never returns them
        # to the pool, a pool leak. The unit tests hang waiting for an item
        # to be returned to the pool. This code is called by the cleanup()
        # method in this file. A call to cleanup() is made in the unit tests
        # in the tearDown() method between test groupings. In the run time
        # code, cleanup() only appears in service.py and only just before doing
        # a sys.exit(), so cleanup() only happens once and the leakage is not a
        # problem. In any case, this pool leak is fixed by the two added
        # statements below.
        self.reply_proxy = None
