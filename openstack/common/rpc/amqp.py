...
class ReplyProxy(ConnectionContext):
    ...
    def __init__(self, conf, connection_pool):
        ...
        self._reply_id = 'reply_' + uuid.uuid4().hex
        super(ReplyProxy, self).__init__(conf, connection_pool, pooled=False)
        self.declare_direct_consumer(self._reply_id, self._process_data)
        self.consume_in_thread()

    def _process_data(self, message_data):
        #rpc_common._safe_log(LOG.debug, _('reply received %s'), message_data)
        ...
        if not waiter:
            ...
            #print 'DEBUG: ReplyProxy: ' + str(message_data)
...
def msg_reply(conf, msg_id, reply_id, connection_pool, reply=None, failure=None,
    ...
    with ConnectionContext(conf, connection_pool) as conn:
        ...
        if ending:
            msg['ending'] = True
        # If a reply_id exists, use it for the response queue,
        # otherwise use the msg_id for backward compatibilty
        if reply_id:
            msg['_msg_id'] = msg_id
            conn.direct_send(reply_id, rpc_common.serialize_msg(msg))
        else:
...
def multicall(conf, context, topic, msg, timeout, connection_pool):
    ...
    # For amqp_rpc_enable_fast_single_reply_queue = False,
    # Can't use 'with' for multicall, as it returns an iterator
    # that will continue to use the connection.  When it's done,
    # connection.close() will get called which will put it back into
    # the pool
    # For amqp_rpc_enable_fast_single_reply_queue = True,
    ...
    if not conf.amqp_rpc_enable_fast_single_reply_queue:
        ...
        msg.update({'_reply_id': connection_pool.reply_proxy.get_reply_id()})
