...
class _DuplicateMsgIdCache(object):
    ...
    def __init__(self, **kwargs):
        ...
        # NOTE: This indicates call other parent class if _DuplicateMsgIdCache
        #       is inherited with other classes, otherwise, do not call
    ...
    def check_duplicated_message(self, message_data):
        ...
        """
        if '_unique_id' in message_data:
            msg_id = message_data['_unique_id']
        else:
...
def _add_unique_id(msg):
    """Add unique_id for checking duplicated messages."""
    if isinstance(msg, dict):
        _unique_id = uuid.uuid4().hex
        msg.update({'_unique_id': _unique_id})
        LOG.debug(_('CHECK DUPLICATED MESSAGE ID is %s.') % (_unique_id))
    else:
        LOG.warning(_('Cannot update _unique_id to %s.') % msg)
