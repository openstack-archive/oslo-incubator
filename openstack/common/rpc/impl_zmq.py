...
from openstack.common.gettextutils import _, _LD, _LE, _LI  # noqa
...
def _deserialize(data):
    ...
    LOG.debug(_LD("Deserializing: %s"), data)
...
class ZmqSocket(object):
    ...
    def __init__(self, addr, zmq_type, bind=True, subscribe=None):
        ...
        LOG.debug(_LD("Connecting to %(addr)s with %(type)s"), str_data)
        LOG.debug(_LD("-> Subscribed to %(subscribe)s"), str_data)
        LOG.debug(_LD("-> bind: %(bind)s"), str_data)
    ...
    def subscribe(self, msg_filter):
        ...
        LOG.debug(_LD("Subscribing to %s"), msg_filter)
...
class InternalContext(object):
    ...
    def _get_response(self, ctx, proxy, topic, data):
        ...
        LOG.debug(_LD("Running func with context: %s"), ctx.to_dict())
...
class ZmqReactor(ZmqBaseReactor):
    ...
    def consume(self, sock):
        ...
        data = sock.recv()
        LOG.debug(_LD("CONSUMER RECEIVED DATA: %s"), data)
...
class Connection(rpc_common.Connection):
    ...
    def create_consumer(self, topic, proxy, fanout=False):
        ...
        LOG.debug(_LD("Consumer is a zmq.%s"),
...
def _call(addr, context, topic, msg, timeout=None,
    ...
    LOG.debug(_LD("Creating payload"))
    ...
    LOG.debug(_LD("Creating queue socket for reply waiter"))
    ...
    with Timeout(timeout, exception=rpc_common.Timeout):
        try:
            ...
            LOG.debug(_LD("Sending cast"))
            _cast(addr, context, topic, payload, envelope)

            LOG.debug(_LD("Cast sent; Waiting reply"))
            # Blocks until receives reply
            msg = msg_waiter.recv()
            LOG.debug(_LD("Received message: %s"), msg)
            LOG.debug(_LD("Unpacking response"))
...
def _multi_send(method, context, topic, msg, timeout=None,
    ...
    """
    conf = CONF
    LOG.debug(_LD("%(msg)s") % {'msg': ' '.join(map(pformat, (topic, msg)))})

    queues = _get_matchmaker().queues(topic)
    LOG.debug(_LD("Sending message(s) to: %s"), queues)
