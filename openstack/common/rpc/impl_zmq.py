...
class ZmqClient(object):
    ...
    def cast(self, msg_id, topic, data, envelope=False):
        ...
        zmq_msg = reduce(lambda x,y: x + y,
            zip(rpc_envelope.keys(), map(bytes, rpc_envelope.values())))
        self.outq.send(map(bytes, (msg_id, topic, 'impl_zmq_v2', data[0]) + zmq_msg))
...
def unflatten_envelope(packenv):
    return dict(zip([ packenv[z] for z in range(len(packenv))
                      if z % 2 == 0 ],
                    [ packenv[z] for z in range(len(packenv))
                      if z % 2 == 1 ]))
    ...
    with Timeout(timeout, exception=rpc_common.Timeout):
        try:
            ...
            if msg[2] == 'cast':  # Legacy version
                ...
                raise UnsupportedRpcEnvelopeVersion(
