...
def _monkey_patch_socket_init(self, context, socket_type):
    super(zmq.Socket, self).__init__(context, socket_type)

    self._eventlet_send_event = zmq._BlockedThread()
    self._eventlet_recv_event = zmq._BlockedThread()
    self._eventlet_send_lock = zmq._QueueLock()
    self._eventlet_recv_lock = zmq._QueueLock()
    ...
    def event(fd):
        ...
        if not send_wake and not recv_wake:
            ...
            zmq._Socket_getsockopt(self, zmq.EVENTS)

    hub = eventlet.hubs.get_hub()
    self._eventlet_listener = hub.add(hub.READ, self.getsockopt(zmq.FD), event)


def _monkey_patch_socket_getsockopt(self, option):
    result = zmq._Socket_getsockopt(self, option)
    if option == zmq.EVENTS:
