...
def exceptions_logged(method):
    ...
    The caller can add an extra kwarg param to the method being decorated:

    _exc_log = False

    When that is passed, exception logging is disabled.
    ...
    def common_inner(topic, msg, *args, **kwargs):
        _exc_log = kwargs.pop('_exc_log', True)
        if not _exc_log:
