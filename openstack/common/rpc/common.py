...
def serialize_msg(raw_msg, topic=None):
    ...
    if topic is not None and CONF.secure_messages.enabled:
        try:
            ...
            if CONF.secure_messages.enforced:
                raise
...
def deserialize_msg(msg):
    ...
    if _SIGNATURE_KEY in msg:
        ...
        if CONF.secure_messages.enabled and CONF.secure_messages.enforced:
            raise InvalidRPCEnvelope(_('Missing Required Signature'))

    if raw_msg is None:
        raw_msg = msg[_MESSAGE_KEY]
