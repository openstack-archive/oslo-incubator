...
class SEKstore(object):
    ...
    Encryption Keys and Encrypted SEK Tickets.
...
class KDSClient(object):
    ...
    def get_ticket(self, request, url = None, redirects = 10):
        ...
        req_kwargs['data'] = json.dumps({'request': request})
...
class SecureMessage(object):
    ...
    :param myname: The endpoint name, tihs is used to source signing keys, and
                   verify incoming messages.
    :param mykey: (optional) explicitly pass in endpoint private key.
    ...
    def _get_ticket(self, target):
        ...
        md['nonce'] = os.urandom(8)
        metadata = base64.b64encode(json.dumps(md))

        # sign metadata
        signature = self.crypto.sign(rkey, metadata)
        ...
        if signature != reply['signature']:
            raise InvalidEncryptedTicket()
        md = base64.b64decode(reply['metadata'])
        ...
        if md['encryption']:
            ...
            tkt = base64.b64decode(reply['ticket'])
        tkt = jsonutils.loads(tkt)
        return tkt['skey'], tkt['ekey'], tkt['esek'], md['expiration']
    ...
    def _get_sek(self, target):
        ...
        sek = store.sek
        if sek is None:
            skey, ekey, esek, expire = self._get_ticket(target)
            store.put(expire, skey, ekey, esek)
            sek = (skey, ekey, esek)
        return sek
