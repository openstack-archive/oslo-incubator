...
class DBAPI(object):
    ...
    def __init__(self, backend_mapping, conf):
        self.__db_api = None
        self.__backend_mapping = backend_mapping
        self.__conf = conf
        if 'use_tpool' not in self.__conf.database:
            self.__conf.register_opts(self.tpool_opts, 'database')

    @property
    def _db_api(self):
        if not self.__db_api:
            db_api = common_db_api.DBAPI(
                self.__conf.database.backend,
                backend_mapping=self.__backend_mapping)
            if self.__conf.database.use_tpool:
