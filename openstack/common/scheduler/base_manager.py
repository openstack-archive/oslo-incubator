...
from openstack.common.scheduler import base_service_manager
...
class BaseSchedulerManager(base_service_manager.BaseManager):
    ...
    def __init__(self, scheduler_driver=None, *args, **kwargs):
        super(BaseSchedulerManager, self).__init__(service_name='scheduler',
                                                   *args, **kwargs)
        sceduler_default = '%(self.component_name)s.scheduler.'\
                           'filter_scheduler.FilterScheduler' % locals()
        scheduler_driver_opt = cfg.StrOpt('scheduler_driver',
                                          default=sceduler_default,
                                          help='Default scheduler driver '
                                          'to use')
        self.conf.register_opt(scheduler_driver_opt)
        if not scheduler_driver:
            scheduler_driver = self.conf.scheduler_driver
