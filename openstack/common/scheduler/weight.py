...
class BaseWeigher(object):
    ...
    def weigh_objects(self, weighed_obj_list, weight_properties):
        ...
        """
        obj.weight += sum([self._weight_multiplier() *
                           self.weigh_object(x.obj, weight_properties)
...
class BaseWeightHandler(object):
    ...
    def get_matching_classes(self, class_names):
        matching = named.NamedExtensionManager(self.namespace, class_names)

        return [x.plugin for x in matching
                if self._is_correct_class(x.plugin)]
