...
class MemoryBackend(backends.BaseCache):
    ...
    def append(self, key, tail):
        if not isinstance(tail, list):
            raise TypeError('tail must be a list instance')
    ...
    def get_many(self, keys, default=None):
        return super(MemoryBackend, self).get_many(keys, default)

    def set_many(self, data, ttl=0):
        return super(MemoryBackend, self).set_many(data, ttl)

    def unset_many(self, keys):
        return super(MemoryBackend, self).unset_many(keys)
