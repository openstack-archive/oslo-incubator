...
class MemcachedBackend(backends.BaseCache):
    ...
    def exists(self, key):
        ...
        try:
            # NOTE(flaper87) Since there's
            # no `exists` command in memcached
            # we'll try to increment the value
            # of key by 0. Memcached returns a
            # non-zero value if the operation
            # succeeds.
            return bool(self._cache.incr(key, 0))
        except ValueError:
            # NOTE(flaper87): If we try to
            # increment a non integer value
            # memcached will fail with ValueError
            # which, in this case, just means the
            # key exists.
            return True
    ...
    def get_many(self, keys, default=None):
        ...
        for cache_key, key in zip(new_keys, keys):
