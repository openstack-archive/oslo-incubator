...
_code_map = dict(
    ...
    for name1, obj in six.iteritems(vars(http_exc))
...
def from_response(response, **kwargs):
    ...
    """
    detail = kwargs.copy()
    error_kwargs = {'detail': detail}

    detail["request_id"] = response.headers.get("x-compute-request-id")
    if "retry-after" in response.headers:
        detail["retry_after"] = response.headers["retry-after"]
    ...
    if content_type.startswith("application/json"):
        try:
            ...
            if hasattr(body, "keys"):
                ...
                detail["details"] = error.get("details", None)
    elif content_type.startswith("text/"):
        detail["details"] = response.text
