...
class RequestNotifier(base.Middleware):
    ...
    @staticmethod
    def process_request_and_response(request, response,
                                     exception=None, traceback=None):
        payload = {
            'request': dict([(k, v) for k, v in request.environ.iteritems()
    ...
    def __call__(self, req):
        try:
            ...
            self.process_request_and_response(req, None, value, traceback)
            raise
        else:
            self.process_request_and_response(req, response)
        return response
