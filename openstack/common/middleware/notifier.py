...
service_name = cfg.StrOpt('api_service_name',
                          default=None,
                          help=('Service name of resource '
                                'middleware.notifier is metering'))

CONF = cfg.CONF
CONF.register_opt(service_name)
...
class RequestNotifier(base.Middleware):
    ...
    def environ_to_dict(environ):
        """Following PEP 333, server variables are lower case, so don't include
        them.
    ...
    @staticmethod
    def add_service_name(env):
        env['HTTP_X_SERVICE_NAME'] = cfg.CONF.api_service_name
        if not env['HTTP_X_SERVICE_NAME']:
            env['HTTP_X_SERVICE_NAME'] = env['HTTP_HOST']

    def process_request(self, request):
        self.add_service_name(request.environ)
        payload = {
