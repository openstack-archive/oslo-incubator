...
Explorer. IE7 doesn't support CORS, and will deny all cross-domain ajax. IE8 and
IE9 do support CORS via the XDomainRequest, but limit the application developer 
to GET and POST, without support for custom headers, and limited to text/plain. 
This negates virtually all usecases in OpenStack context. 
...
## Note that these domains don't support wildcards of partial matches. This list
## is sent to the browser, and it is enforced by this middleware.
# allow_origin = *
...
## A comma separated list of headers the client is allowed to customize. This is
## sent to the browser as-is; it is not enforced by this middleware
...
## this off. Even when switched off, the CORS headers are added to the response,
## so the pre-flight request will work as intended, but it may trigger 
## unintended side-effects in your implementation.
# hijack_options = true
...
class CorsMiddleware(wsgi.Middleware):
    ...
    def __init__(self, app, conf):        
        allow_origins = set(conf['allow_origin'].split())
    ...
    def process_request(self, req):
        ...
        if origin not in self.allowed_origin and '*' not in self.allowed_origins:
