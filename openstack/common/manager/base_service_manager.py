...
class BaseManager(db_base.Base, periodic_task.PeriodicTasks):
    ...
    def __init__(self, host=None, db_driver=None,
        ...
        if CONF.manager.project_name is None:
            raise exception.Invalid(_("`project_name` option for service "
                                      "should be overwritten."))
        self.project_name = CONF.manager.project_name
        self.load_plugins()
        self.backdoor_port = None
        self.service_name = service_name

    def load_plugins(self):
        load_plugins = CONF.manager.load_plugins
        if load_plugins:
            pluginmgr = pluginmanager.PluginManager(self.project_name,
                                                    self.__class__)
    ...
    def create_rpc_dispatcher(self, **kwargs):
        ...
        serializer = self._get_serializer(**kwargs)
    ...
    def service_version(self, context):
        version_info = pbr_version.VersionInfo(self.project_name)
