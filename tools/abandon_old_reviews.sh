...
# first purge the all reviews that are more than 12w old and blocked by a core -2
...
PROJECTS="(project:^.*openstack/oslo.*)"

blocked_reviews=$(ssh review.openstack.org "gerrit query --current-patch-set --format json $PROJECTS status:open age:12w label:Code-Review<=-2" | jq .currentPatchSet.revision | grep -v null | sed 's/"//g')

blocked_msg=$(cat <<EOF

This review is > 12 weeks without comment and currently blocked by a
...
# then purge all the reviews that are > 12w with no changes and Jenkins has -1ed

failing_reviews=$(ssh review.openstack.org "gerrit query  --current-patch-set --format json $PROJECTS status:open age:12w NOT label:Verified>=1,jenkins" | jq .currentPatchSet.revision | grep -v null | sed 's/"//g')

failing_msg=$(cat <<EOF

This review is > 12 weeks without comment, and failed Jenkins the last
