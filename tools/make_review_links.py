...
NO_MINUS = '-label:Code-Review-1+-label:Code-Review-2+-label:Workflow-1'
PLUS_2 = 'label:Code-Review=2'
NO_PLUS = '-label:Code-Review%253D2+-label:Code-Review%253D1'
NO_PLUS_2 = '-label:Code-Review=2'

LINKS = [
    ('Potential+Merges', [OPEN, VERIFIED, NO_MINUS, PLUS_2]),
    ('No+Objections', [OPEN, VERIFIED, NO_MINUS, NO_PLUS_2]),
    ('No+Reviews', [OPEN, VERIFIED, NO_MINUS, NO_PLUS])
]
...
def main():
    ...
    all_repos = [pr['repo'] for pr in program['projects']]

    # Get the list of repos that do not match the ^.*oslo.* pattern
    repos = [r for r in all_repos if 'oslo' not in r]
    ...
    url = BASE_URL + '/dashboard/?title=Personal+Oslo+Dashboard&' + '&'.join(
        '%s+-owner:self' % s for s in sections
    )
    print('* [%(url)s Personal Oslo Review Dashboard]' % {
        'url': url,
    })
