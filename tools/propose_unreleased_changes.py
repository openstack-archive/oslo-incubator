...
def maybe_create_release(release_repo_path,
    ...
    if response == 'yes':
        ...
        if os.path.exists(newest_release_path):
            newest_release = load_yaml(newest_release_path)
        else:
...
def prettify_yaml(obj):
    return pyaml.dumps(obj, safe=True,
...
def load_yaml(path):
    with open(path, 'rb') as fh:
        ...
        # for what we are loading here in this program...
        return yaml.load(fh.read(), Loader=yamlordereddictloader.Loader)
...
def main():
    ...
    parser.add_argument("-p", "--projects", metavar="FILE",
                        help="Data file containing projects to analyze",
                        required=True)
    parser.add_argument("-r", "--releases", metavar="PATH",
                        help="Release repository path", required=True)
        ...
        with tempdir() as tdir:
            ...
            for project, short_project in project_names:
                ...
                if last_release_path is None or last_release_cycle is None:
                    ...
                    project_releases = load_yaml(last_release_path)
