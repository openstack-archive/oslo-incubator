...
PARSED_OPTIONS=$(getopt -n "${0##*/}" -o hb:p:m:l:o:t: \
                 --long help,base-dir:,package-name:,output-dir:,module:,library:,use-tox-exclude: -- "$@")
...
while true; do
    case "$1" in
        -h|--help)
            ...
            echo "-t, --use-tox-exclude       add find excludes for paths found in tox.ixi exclude lines"
            exit 0
            ;;
        -b|--base-dir)
            shift
