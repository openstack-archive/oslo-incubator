    ...
    for p in projects:
        ...
        for base in packaging.get_requirements_files():
            ...
            for req in packaging.parse_requirements([req_file]):
                try:
                    req_name = pkg_resources.Requirement.parse(req).project_name
                except ValueError:
                    continue
...
def main():
    ...
    conf.register_cli_opt(
        cfg.StrOpt(
            ...
            short='l',
            default='',
            help='the name of the library being tested; defaults to current dir',
        )
    )
    conf.register_cli_opt(
        cfg.BoolOpt(
    ...
    conf.register_cli_opt(
        cfg.MultiStrOpt(
            ...
            short='e',
            default=['py27', 'pep8'],
            help='the name of the tox environment to test; defaults to py27 and pep8',
        )
    )
    conf.register_cli_opt(
        cfg.MultiStrOpt(
    ...
    if not projects:
        # TODO: Need to update this to look at gerrit, so we can check
        # out the projects we want to test with.
    ...
    after = len(projects)
    if after < before:
        print('ignoring %s projects that do not use %s' % (before - after, lib_name))
    ...
    for p in projects:
        ...
        for e in conf.env:
            log_name = 'cross-test-%s-%s.log' % (p.split('/')[-1].replace('.', '-'), e)
            with open(log_name, 'w') as log_file:
                print('testing %s in %s, logging to %s' % (e, p, log_name), end=' ')
                sys.stdout.flush()
