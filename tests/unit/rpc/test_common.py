...
class TestRPCExceptionLogging(test_utils.BaseTestCase):
    ...
    def test_exceptions_logged_decorator(self):
        ...
        try:
            # First try in 'disabled mode' by passing _log_exc = False
            fake_method('conf', 'context', self.topic, self.msg,
                        _log_exc=False)
            self.assertEqual(self.expected_log_message, 'NOT CALLED')
            # This time it should log (tested in tear down)
            fake_method('conf', 'context', self.topic, self.msg)
        except rpc_common.Timeout:
            pass

    def test_exceptions_logged_decorator_extra_param(self):
        @rpc_common.exceptions_logged
        def fake_method(
