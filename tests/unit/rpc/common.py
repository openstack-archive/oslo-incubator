...
class BaseRpcAMQPTestCase(BaseRpcTestCase):
    ...
    def test_single_reply_queue_on_has_ids(
        ...
        #The correct handling of single_reply_queue_for_callee_off=True
        #in for the multithreaded test is the logging of an error and
        #an RPC timeout.  Uncommenting the following line will produce
        #that scenario.  Since the logging of an error results in a unit
        #test failure, the following line  must be left commented out.
...
def rpc_wrapper(callid, func, *args):
    """This wrapper was added because tests hang when there is a bug that
       caused an RPC to timeout.  The send() event would hang waiting for
