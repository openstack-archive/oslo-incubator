...
class BaseRpcAMQPTestCase(BaseRpcTestCase):
    ...
    def test_single_reply_queue_on_has_ids(self):
        ...
        self.config(amqp_rpc_enable_fast_single_reply_queue=True)
        ...
        class MyReplyProxy(rpc_amqp.ReplyProxy):
            def _process_data(myself, message_data):
                #with open ('/root/oslo-incubator/mylog', 'a') as f:
    ...
    def test_single_reply_queue_off_no_ids(self,
                                       single_reply_queue_for_callee_on=False):
        if not self.rpc:
            raise nose.SkipTest('rpc driver not available.')

        self.config(amqp_rpc_enable_fast_single_reply_queue=False)
        ...
        def my_MulticallWaiter_call(myself, data):
            ...
            self.assertTrue(not '_msg_id' in data)
...
class SyncedEchoCall():
    ...
    def spawn(self, *args, **kwargs):
        ...
        args = list(args)
        value = kwargs['value']
        args.append({"method": "synced_echo", "args": {"value": value, "callid": idx}})
