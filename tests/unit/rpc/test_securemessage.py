...
class RpcCryptoTestCase(test_utils.BaseTestCase):
    ...
    def test_KEYstore(self):
        ...
        keys = store.get_sek('foo', 'bar')
        self.assertIsNone(keys)

        sek = {'skey': 'skey', 'ekey': 'ekey'}

        #add entry in the cache
        store.put_sek('foo', 'bar', 'skey', 'ekey', 2000000000)
        ...
        keys = store.get_sek('bar', 'foo')
        self.assertIsNone(keys)

        #add expired entry in the cache
        store.put_sek('foo', 'bar', 'skey', 'ekey', 1000000000)
    ...
    def _test_secure_message(self, data, encrypt):
        ...
        send = rpc_secmsg.SecureMessage(data['source'], CONF, data['send_key'],
                                        store, encrypt, enctype=data['cipher'],
                                        hashtype=data['hash'])
        recv = rpc_secmsg.SecureMessage(data['target'], CONF, data['recv_key'],
                                        store, encrypt, enctype=data['cipher'],
                                        hashtype=data['hash'])
        ...
        store.put_sek(source, target, data['skey'], data['ekey'], 2000000000)
