...
class RpcCryptoTestCase(test_utils.BaseTestCase):
    ...
    def _test_secure_message(self, data, encrypt):
        ...
        send = rpc_secmsg.SecureMessage(data['source'], data['send_key'],
                                        encrypt, enctype=data['cipher'],
                                        hashtype=data['hash'])
        recv = rpc_secmsg.SecureMessage(data['target'], data['recv_key'],
                                        encrypt, enctype=data['cipher'],
                                        hashtype=data['hash'])
    ...
    def test_secure_message_sha256_aes(self):
        foo_to_bar_sha256_aes = {
            ...
            'hash': 'SHA256',
            'cipher': 'AES',
            'skey': 'L\x85\x11\x9b9\xa6\x9a\xaa]\xcb4\xfa\x9f\xa0\xf7\xeb',
            'ekey': '\x0fS\xcb\xf5x\xe2\x83X%\x0b\xe5\xe5\x92\xd7\x0e4',
    ...
    def test_secure_message_md5_des(self):
        foo_to_baz_md5_des = {
            ...
            'hash': 'MD5',
            'cipher': 'DES',
            'skey': ';\xeaw\xf6d\xd6\xa1I',
            'ekey': "$\xc4\xaf\xa0\x1d\xee'\x8a",
