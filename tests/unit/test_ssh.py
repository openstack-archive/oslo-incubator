...
class SSHSFTPTestCase(test.BaseTestCase):
    ...
    def test_download(self):
        ...
        calls = [
            mock.call.get('src', 'dest'),
            mock.call.close(),
        ]
        self.assertEqual(calls, self.ftp.mock_calls)
        self.client.close.assert_called_once()

    def test_upload_expanduser(self):
        self.ssh.upload('~/src', '~/dst')
        src, dst = self.ftp.put.mock_calls[0][1]
        self.assertTrue(src.endswith('/src'))
        self.assertTrue(dst.endswith('/dst'))

    def test_download_expanduser(self):
        self.ssh.download('~/src', '~/dst')
        src, dst = self.ftp.get.mock_calls[0][1]
        self.assertTrue(src.endswith('/src'))
        self.assertTrue(dst.endswith('/dst'))
