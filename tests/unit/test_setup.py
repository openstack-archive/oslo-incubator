...
class GitLogsTest(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.useFixture(DiveDir(temp_dir_fixture.path))
        # fake: "we are in git repository" (since isdir(".git") returns True)
        self.useFixture(fixtures.MonkeyPatch("os.path.isdir",
                                             lambda path: True))

    def test_write_git_changelog(self):
        self.useFixture(fixtures.FakePopen(lambda _: {
            'stdout': BytesIO("Author: Foo Bar <email@bar.com>\n")
        }))
