...
class TestBaseModel(utils.BaseTestCase):
    ...
    def test_data_as_sequence_is_handled_properly(self):
        ...
        # now, if we don't handle Arrays properly, we'll get a TyperError here
        self.assertEquals(str(model), '0: a;1: b;')

    def test_immutable_mapping_uses_cow(self):
        ...
        self.assertEquals(str(model), 'a: 2;c: 8;b: 4;')

        model['d'] = 16

        self.assertEquals(str(model), 'a: 2;c: 8;b: 4;d: 16;')
        self.assertEquals(mp.data, {'a': 2, 'b': 4, 'c': 8}) 
