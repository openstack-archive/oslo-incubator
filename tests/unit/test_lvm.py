...
class TestLVMWrapper(test.BaseTestCase):
    ...
    def _fake_lvchange_help_with_ignoreactivation(self, *cmd, **kwargs):
        output = """
    ...
    def _fake_lvchange_help_without_ignoreactivation(self, *cmd, **kwargs):
        return ("""
              WARNING: Running as a non-root user. Functionality may be
              unavailable.
              lvchange: Change the attributes of logical volume(s)
    ...
    def test_get_vg_uuid(self):
        ...
        self.assertEqual(self.vg_ref._get_vg_uuid(), expected_result)
    ...
    @mock.patch('openstack.common.processutils.execute')
    def test_get_lvm_version(self, _mock_execute):
        _mock_execute.side_effect = self._fake_lvm_version
        self.assertEqual((2, 2, 95),
                         self.vg_ref.get_lvm_version(self.root_helper))

    @mock.patch('openstack.common.processutils.execute')
    def test_lvm_thin_support(self, _mock_execute):
        _mock_execute.side_effect = self._fake_lvm_version
        self.assertTrue(
            self.vg_ref.supports_thin_provisioning(self.root_helper))

        _mock_execute.side_effect = self._fake_lvm_version_old
        self.assertFalse(
            self.vg_ref.supports_thin_provisioning(self.root_helper))

    @mock.patch('openstack.common.processutils.execute')
    def test_snapshot_activiation_support(self, _mock_execute):
    ...
    @mock.patch('openstack.common.processutils.execute')
    def test_get_all_volumes(self, _mock_execute):
        _mock_execute.side_effect = self._fake_lvs
        ...
        self.assertEqual(expected, vols)

    @mock.patch('openstack.common.processutils.execute')
    def test_get_volume(self, _mock_execute):
        expected =\
            {'name': 'volume-4164e916-d2ef-4b01-b401-2b8564b3dab1',
             'size': '1.00g', 'vg': 'stack-vg2'}
    ...
    @mock.patch('openstack.common.processutils.execute')
    def test_get_all_volume_groups(self, _mock_execute):
        _mock_execute.side_effect = self._fake_vgs
        expected = [
            {'name': 'precise64',
             'size': '79.76g',
             'available': '0g',
    ...
    @mock.patch('openstack.common.processutils.execute')
    def test_get_all_physical_volumes(self, _mock_execute):
        _mock_execute.side_effect = self._fake_pvs
        expected = [
        ...
        self.assertEqual(expected, pvs)

    @mock.patch('openstack.common.processutils.execute')
    def test_update_volume_group_info(self, _mock_execute):
        _mock_execute.side_effect = self._fake_get_vg2
        ...
        self.assertEqual('0.00g', self.vg_ref.vg_free_space)
        self.assertEqual('10', self.vg_ref.vg_lv_count)
        self.assertEqual('1000.00g', self.vg_ref.vg_size)

        # Now run the update and make sure it's reflected correctly
        self.vg_ref.update_volume_group_info()
        self.assertEqual('0.43g', self.vg_ref.vg_free_space)
        self.assertEqual('2', self.vg_ref.vg_lv_count)
        self.assertEqual('10.01g', self.vg_ref.vg_size)
    ...
    @mock.patch('openstack.common.processutils.execute')
    def test_snapshot_activate_support(self, _mock_execute):
        _mock_execute.side_effect = self._fake_lvm_version
        self.vg_ref._supports_snapshot_lv_activation = None
        self.assertTrue(self.vg_ref.supports_snapshot_lv_activation)
    ...
    def test_lvchange_ignskipact_support_yes(self):
        self.vg_ref._supports_lvchange_ignoreskipactivation = None
        self.output = self._fake_lvchange_help_with_ignoreactivation()

        self.assertTrue(self.vg_ref.supports_lvchange_ignoreskipactivation)
        self.vg_ref._supports_lvchange_ignoreskipactivation = None

    def test_lvchange_ignskipact_support_no(self):
        self.vg_ref._supports_lvchange_ignoreskipactivation = None
        self.output = self._fake_lvchange_help_without_ignoreactivation()

        self.assertFalse(self.vg_ref.supports_lvchange_ignoreskipactivation)
        self.vg_ref._supports_lvchange_ignoreskipactivation = None
