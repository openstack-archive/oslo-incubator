...
from openstack.common.processutils import InvalidArgumentError
...
class TestPasswordUtils(test.BaseTestCase):
    ...
    def test_generate_password(self):
        ...
        self.assertEqual(len(password), passwordutils.DEFAULT_PASSWORD_LENGTH)
    ...
    def test_generate_password_uniqueness(self):
        ...
        for i in xrange(20):
            ...
            self.assertEqual(len(password_list[i]),
                             passwordutils.DEFAULT_PASSWORD_LENGTH)

        self.assertEqual(len(password_list), len(set(password_list)))

    def test_raises_invalid_argument_error(self):
        # DEFAULT_PASSWORD_SYMBOLS length is 3
        self.assertRaises(InvalidArgumentError,
                          passwordutils.generate_password, 0)

        self.assertRaises(InvalidArgumentError,
                          passwordutils.generate_password, 1)

        self.assertRaises(InvalidArgumentError,
                          passwordutils.generate_password, 2)

        self.assertRaises(InvalidArgumentError,
                          passwordutils.generate_password, 3)
