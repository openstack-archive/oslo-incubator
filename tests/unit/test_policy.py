...
class RulesTestCase(test_base.BaseTestCase):
    ...
    def test_init(self):
        rules = parser.Rules(dict(a=1, b=2, c=3), 'a')

        self.assertEqual(rules, dict(a=1, b=2, c=3))
        self.assertEqual(rules.default_rule, 'a')

    def test_no_default(self):
        rules = parser.Rules(dict(a=1, b=2, c=3))

        self.assertRaises(KeyError, lambda: rules['d'])

    def test_missing_default(self):
        rules = parser.Rules(dict(a=1, c=3), 'b')

        self.assertRaises(KeyError, lambda: rules['d'])

    def test_with_default(self):
        rules = parser.Rules(dict(a=1, b=2, c=3), 'b')
    ...
    @mock.patch.object(parser, 'parse_rule', lambda x: x)
    def test_load_json(self):
        exemplar = """{
    "admin_or_owner": [["role:admin"], ["project_id:%(project_id)s"]],
    "default": []
}"""
        rules = parser.Rules.load_json(exemplar, 'default')
    ...
    "admin_or_owner": ""
        ...
        rules = parser.Rules(dict(
            admin_or_owner=parser.TrueCheck(),
        ))
...
class EnforcerTest(PolicyBaseTestCase):
    ...
    def test_rule_with_check(self):
        ...
        rules = parser.Rules.load_json(rules_json)
...
class FakeCheck(parser.BaseCheck):
...
class CheckFunctionTestCase(PolicyBaseTestCase):
    ...
    def test_check_raises(self):
        self.enforcer.set_rules(dict(default=parser.FalseCheck()))
...
class CheckForTest(parser.Check):
    def __call__(self, target, creds, enforcer):
        pass
...
class AndCheckTestCase(test_base.BaseTestCase):
    def test_init(self):
        check = parser.AndCheck(['rule1', 'rule2'])
...
class OrCheckTestCase(test_base.BaseTestCase):
    def test_init(self):
        check = parser.OrCheck(['rule1', 'rule2'])
...
class ParseCheckTestCase(test_base.BaseTestCase):
    def test_false(self):
        result = parser._parse_check('!')

        self.assertTrue(isinstance(result, parser.FalseCheck))

    def test_true(self):
        result = parser._parse_check('@')

        self.assertTrue(isinstance(result, parser.TrueCheck))

    def test_bad_rule(self):
        result = parser._parse_check('foobar')

        self.assertTrue(isinstance(result, parser.FalseCheck))

    @mock.patch.object(parser, '_checks', {})
    def test_no_handler(self):
        result = parser._parse_check('no:handler')
    ...
    def test_check(self):
        result = parser._parse_check('spam:handler')

        self.assertEqual(result, 'spam_check')
        parser._checks['spam'].assert_called_once_with('spam', 'handler')
        self.assertFalse(parser._checks[None].called)
    ...
    def test_check_default(self):
        ...
        parser._checks[None].assert_called_once_with('spam', 'handler')


class ParseListRuleTestCase(test_base.BaseTestCase):
    def test_empty(self):
        result = parser._parse_list_rule([])

        self.assertTrue(isinstance(result, parser.TrueCheck))
        self.assertEqual(str(result), '@')

    @mock.patch.object(parser, '_parse_check', FakeCheck)
    ...
    def test_oneele_bare(self):
        result = parser._parse_list_rule(['rule'])
    ...
    def test_oneele_multi(self):
        result = parser._parse_list_rule([['rule1', 'rule2']])

        self.assertTrue(isinstance(result, parser.AndCheck))
    ...
    def test_multi_oneele(self):
        ...
        self.assertTrue(isinstance(result, parser.OrCheck))
        self.assertEqual(len(result.rules), 2)
        for i, value in enumerate(['rule1', 'rule2']):
            self.assertTrue(isinstance(result.rules[i], FakeCheck))
            self.assertEqual(result.rules[i].result, value)
    ...
    def test_multi_multi(self):
        ...
        for i, values in enumerate([['rule1', 'rule2'], ['rule3', 'rule4']]):
            self.assertTrue(isinstance(result.rules[i], parser.AndCheck))
...
class ParseTokenizeTestCase(test_base.BaseTestCase):
    @mock.patch.object(parser, '_parse_check', lambda x: x)
    def test_tokenize(self):
        ...
        result = list(parser._parse_tokenize(exemplar))
...
class ParseStateMetaTestCase(test_base.BaseTestCase):
    def test_reducer(self):
        @parser.reducer('a', 'b', 'c')
        @parser.reducer('d', 'e', 'f')
        def spam():
            pass
    ...
    def test_parse_state_meta(self):
        @six.add_metaclass(parser.ParseStateMeta)
...
class ParseStateTestCase(test_base.BaseTestCase):
    ...
    @mock.patch.object(parser.ParseState, 'reducers', [(['tok1'], 'meth')])
    @mock.patch.object(parser.ParseState, 'meth', create=True)
    ...
    @mock.patch.object(parser.ParseState, 'reducers',
                       [(['tok1', 'tok2'], 'meth')])
    @mock.patch.object(parser.ParseState, 'meth', create=True)
    ...
    @mock.patch.object(parser.ParseState, 'meth1', create=True,
                       return_value=[('tok4', 'val4')])
    @mock.patch.object(parser.ParseState, 'meth2', create=True,
    ...
    def test_shift(self):
        ...
        with mock.patch.object(parser.ParseState, 'reduce') as mock_reduce:
    ...
    @mock.patch.object(parser, 'AndCheck', lambda x: x)
    ...
    @mock.patch.object(parser, 'OrCheck', lambda x: x)
    ...
    @mock.patch.object(parser, 'NotCheck', lambda x: 'not %s' % x)
...
class ParseTextRuleTestCase(test_base.BaseTestCase):
    def test_empty(self):
        result = parser._parse_text_rule('')

        self.assertTrue(isinstance(result, parser.TrueCheck))

    @mock.patch.object(parser, '_parse_tokenize',
                       return_value=[('tok1', 'val1'), ('tok2', 'val2')])
    @mock.patch.object(parser.ParseState, 'shift')
    @mock.patch.object(parser.ParseState, 'result', 'result')
    def test_shifts(self, mock_shift, mock_parse_tokenize):
        result = parser._parse_text_rule('test rule')
    ...
    @mock.patch.object(parser, '_parse_tokenize', return_value=[])
...
class ParseRuleTestCase(test_base.BaseTestCase):
    @mock.patch.object(parser, '_parse_text_rule', return_value='text rule')
    @mock.patch.object(parser, '_parse_list_rule', return_value='list rule')
    def test_parse_rule_string(self, mock_parse_list_rule,
                               mock_parse_text_rule):
        result = parser.parse_rule("a string")
    ...
    def test_parse_rule_list(self, mock_parse_list_rule, mock_parse_text_rule):
        result = parser.parse_rule([['a'], ['list']])
...
class CheckRegisterTestCase(test_base.BaseTestCase):
    ...
    def test_register_check(self):
        class TestCheck(parser.Check):
            pass

        parser.register('spam', TestCheck)

        self.assertEqual(parser._checks, dict(spam=TestCheck))

    @mock.patch.object(parser, '_checks', {})
    def test_register_check_decorator(self):
        @parser.register('spam')
        class TestCheck(parser.Check):
            pass
...
class RuleCheckTestCase(test_base.BaseTestCase):
    ...
    def test_rule_missing(self):
        check = parser.RuleCheck('rule', 'spam')
...
class RoleCheckTestCase(PolicyBaseTestCase):
    def test_accept(self):
        check = parser.RoleCheck('role', 'sPaM')
...
class HttpCheckTestCase(PolicyBaseTestCase):
    ...
    def test_accept(self, mock_urlopen):
        check = parser.HttpCheck('http', '//example.com/%(name)s')
...
class GenericCheckTestCase(PolicyBaseTestCase):
    def test_no_cred(self):
        check = parser.GenericCheck('name', '%(name)s')
    ...
    def test_constant_string_mismatch(self):
        check = parser.GenericCheck("'spam'", '%(name)s')
    ...
    def test_constant_literal_mismatch(self):
        check = parser.GenericCheck("True", '%(enabled)s')
