...
from openstack.common import imageutils as images
...
class ImageUtilsTestCase(utils_test.BaseTestCase):
    def tearDown(self):
        self.mox.UnsetStubs()
        super(ImageUtilsTestCase, self).tearDown()

    def test_qemu_img_info_file_absent(self):
        path = "disk.blahblah"
        self.mox.StubOutWithMock(os.path, 'exists')
        os.path.exists(path).AndReturn(False)
        self.mox.ReplayAll()
        ...
        expected = ('image: None\n'
                    ...
                    'cluster_size: None\n'
                    'backing_file: None')
        self.assertEqual(str(image_info), expected)
...
"""
        ...
        utils.execute('env', 'LC_ALL=C', 'LANG=C',
                      'qemu-img', 'info', path,
                      run_as_root=True).AndReturn((example_output, ''))
        self.mox.ReplayAll()
        image_info = images.qemu_img_info(path)
        self.mox.VerifyAll()
