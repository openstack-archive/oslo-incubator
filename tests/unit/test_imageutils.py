...
class ImageUtilsRawTestCase(test.BaseTestCase):
    ...
    def _init_img_info(self, image, file_format, virtual_size, disk_size):
        return ('image: %s' % image,
                'file_format: %s' % file_format,
                'virtual_size: %s' % virtual_size,
                'disk_size: %s' % disk_size)

    def _insert_snapshots(self, img_info, snapshot_count):
    ...
    def _base_validation(self, image_info, image, file_format,
                         virtual_size, disk_size, snapshot_count):
        self.assertEqual(image_info.image, image)
        self.assertEqual(image_info.file_format, file_format)
        self.assertEqual(image_info.virtual_size, virtual_size)
        self.assertEqual(image_info.disk_size, disk_size)
        if snapshot_count is not None:
            self.assertEqual(len(image_info.snapshots), snapshot_count)

    def test_qemu_img_info(self):
        img_info = self._init_img_info(self.image_name, self.file_format,
                                       self.virtual_size, self.disk_size)
        if self.garbage_before_snapshot is True:
            img_info = img_info + ('blah BLAH: bb',)
        if self.snapshot_count is not None:
            img_info = self._insert_snapshots(img_info, self.snapshot_count)
        if self.garbage_before_snapshot is False:
            img_info = img_info + ('junk stuff: bbb',)
        example_output = '\n'.join(img_info)
        image_info = imageutils.QemuImgInfo(example_output)
        self._base_validation(image_info, self.image_name, self.file_format,
                              self.exp_virtual_size, self.exp_disk_size,
                              self.snapshot_count)
