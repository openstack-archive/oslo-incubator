...
class GettextTest(utils.BaseTestCase):
    ...
    def test_get_available_languages(self):
        ...
        def _mock_list_locale_data():
            return ['zh', 'es', 'nl', 'fr']
        self.stubs.Set(localedata, "list", _mock_list_locale_data)

        # Only the languages available for a specific translation domain
        def _mock_gettext_find(domain, localedir=None, languages=[]):
            if domain == 'test_domain':
                return True if any(x in ['zh', 'es']
