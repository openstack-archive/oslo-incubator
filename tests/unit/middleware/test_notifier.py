...
class NotifierMiddlewareTest(utils.BaseTestCase):
    ...
    def setUp(self):
        ...
        self.middleware = notifier.RequestNotifier(FakeApp())
    ...
    def test_notification(self):
        ...
        with mock.patch('openstack.common.notifier.api.notify') as notify:
            ...
            self.assertEqual(request['environ']['PATH_INFO'], '/foo/bar')
    ...
    def test_notification_response_failure(self):
        self.middleware = notifier.RequestNotifier(FakeFailingApp())
        req = webob.Request.blank('/foo/bar',
                                  environ={'REQUEST_METHOD': 'GET'})
        with mock.patch('openstack.common.notifier.api.notify') as notify:
            try:
