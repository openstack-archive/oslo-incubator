...
def custom_http_notify(url, body):
    LOG = log.getLogger('openstack.common.notifier.http_notifier')
    LOG.debug("Custom notifier")
...
class HTTPNotifierTestCase(test_utils.BaseTestCase):
    ...
    def test_custom_notifier(self):
        self._stub_out_http_request(self.stubs, 200)
        # import the module to set the http_notifier_method.
        try:
            importutils.import_module(self.notification_driver)
        except ImportError:
            self.fail("Failed to load notification driver %s" %
                      self.notification_driver)
        self.config(http_notifier_method="tests.unit.notifier."
                        "test_http_notifier.custom_http_notify")
        notifier_api.notify(self._dummy_context, 'publisher_id',
                            self.notification_event, "INFO", dict(a=3))

        msg = "Custom http_notifier_method not getting called."
        self.assertEqual(self.debug, "Custom notifier", msg)
