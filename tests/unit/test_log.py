...
"""
    ...
    def setUp(self):
        ...
        self.config = self.useFixture(config.Config()).config
        self._create_tempfile = fileutils.write_to_tempfile

    def tearDown(self):
        super(LogConfigTestCase, self).tearDown()
        fileutils.delete_if_exists(self.log_config_append)

    def test_log_config_append_ok(self):
        self.log_config_append = \
            self._create_tempfile(content=self.minimal_config,
                                  prefix='logging',
                                  suffix='.conf'
                                  )
    ...
    def test_log_config_append_invalid(self):
        self.log_config_append = \
            self._create_tempfile(content=self.minimal_config[5:],
                                  prefix='logging',
                                  suffix='.conf'
                                  )
        self.config(log_config_append=self.log_config_append)
