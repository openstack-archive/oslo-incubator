...
class TestDBAPI(object):
    ...
    def flags(self, **kw):
        ...
        group = kw.pop('group', 'database')
        for k, v in kw.iteritems():
            api.CONF.set_override(k, v, group)
...
class DBReconnectTestCase(test_utils.BaseTestCase):
    ...
    def test_retry_one(self):
        self.dbapi.flags(db_retry_interval=1)
        try:
    ...
    def test_retry_two(self):
        self.dbapi.flags(db_inc_retry_interval=False)
        self.dbapi.flags(db_retry_interval=1)
    ...
    def test_retry_until_failure(self):
        self.dbapi.flags(db_inc_retry_interval=False)
        self.dbapi.flags(db_max_retries=3)
        self.dbapi.flags(db_retry_interval=1)
