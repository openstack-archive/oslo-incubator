...
class TestDBAPI(object):
    ...
    @api.disable_db_retry
    def api_raise_disable_retry(self, *args, **kwargs):
        return self._api_raise(*args, **kwargs)
...
class DBReconnectTestCase(test_utils.BaseTestCase):
    ...
    def test_retry_one(self):
        ...
        try:
            func = self.dbapi.api_raise_default
    ...
    def test_retry_enabled_by_decorator(self):
        self.config(group='database', db_retry_interval=1)
        try:
    ...
    def test_retry_disabled_by_decorator(self):
        self.config(group='database', use_db_reconnect=True)
        self.test_db_api.error_counter = 2
        self.assertRaises(
            exception.DBConnectionError,
            self.dbapi.api_raise_disable_retry,
        )

        self.assertEqual(
            1, self.test_db_api.error_counter,
            'Counter decremented, retry disable logic probably failed.')
