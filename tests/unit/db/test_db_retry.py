...
class DBReconnectTestCase(test_utils.BaseTestCase):
    ...
    def sqlite_raise(self):
        """Simulate raising a database-has-gone-away error for postgresql."""
        exc = sqlite3.dbapi2.ProgrammingError(
            'Cannot operate on a closed database.', False, False)
        raise exc

    def test_raise_db_conn_error_sqlite(self):
        try:
            self.sqlite_raise()
        except Exception as e:
            self.assertRaises(
                exception.DBConnectionError,
                session._raise_if_db_connection_lost, e, session.get_engine(),
            )
