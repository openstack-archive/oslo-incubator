...
class MySQLTraditionalModeTestCase(test_base.MySQLdbTestCase):
    ...
    def setUp(self):
        ...
        self.conf.set_override('mysql_traditional_mode',
                               True, group='database')

        if self._is_backend_avail():
            self.engine = session.get_engine()
            self.connection = self.engine.connect()
        else:
            backend = self.conf.database.connection.split(':')[0]
            self.skip('%s backend is not available.' % backend)
    ...
    def test_string_too_long(self):
        with self.connection.begin():
            # NOTE: if ORM used DBError wrapper will be raised.
