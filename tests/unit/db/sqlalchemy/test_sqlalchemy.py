...
class MysqlSetCallbackTest(test_log.LogTestBase):
    ...
    def _call_set_callback(self, listen_mock, sql_mode=None, realmode=None):
        ...
        session._mysql_set_callback(engine, sql_mode=sql_mode)
        return engine

    def test_set_mode_traditional(self):
        # If _mysql_set_callback is called with an sql_mode, then, in the
        # 'checkout' callback, the SQL mode is set on the connection.
    ...
    def test_set_mode_no_mode(self):
        # If _mysql_set_callback is called with sql_mode=None, then, in the
        # 'checkout' callback, the SQL mode is NOT set on the connection.
