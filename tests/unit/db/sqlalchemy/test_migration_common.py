...
class TestMigrationCommon(test_base.DbTestCase):
    ...
    def test_db_version_control(self):
        with mock.patch.object(migration,
                               '_find_migrate_repo') as mock_find_repo:
            with mock.patch.object(versioning_api,
                                   'version_control') as mock_version_control:
    ...
    def test_db_sync_sanity_called(self):
        with mock.patch.object(migration,
                               '_find_migrate_repo') as mock_find_repo:
            with mock.patch.object(migration,
                                   '_db_schema_sanity_check') as mock_sanity:
                with mock.patch.object(versioning_api,
                                       'downgrade'):
                    mock_find_repo.return_value = self.return_value
