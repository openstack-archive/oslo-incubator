...
class TestSync(test_utils.BaseTestCase, migrate.SyncModelsWithMigrations):
    ...
    def _test_sync_models_with_db(self, base, errors=None):
        ...
        engine.dispose()
        if errors is not None:
            self.assertRaises(MismatchError, self._test_list_of_tables,
                              self.DIALECT, test_mode=True)
    ...
    def test_sync_extra_tables(self):
        errors = ['Tables in `models`  have a difference '
                  'with migrations : (fake_table,extra_table).']
