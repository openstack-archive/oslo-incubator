...
class TestSyncMySQL(test_utils.BaseTestCase, migrate.SyncModelsWithMigrations):
    ...
    def _create_database(self, dialect, user, password, database,
                         host='localhost'):
        """We should create tables from checked models in temporary database.
        """
        super(TestSyncMySQL, self)._create_database(dialect, user, password,
                                                    database, host)
        conn_string = migrate._get_connect_string(dialect,
                                                  user,
                                                  password,
                                                  database)
    ...
    def _test_sync_models_with_db(self, base, errors=None, exclude=[]):
        ...
        if errors is not None:
            ...
            self._test_sync_models(self.DIALECTS[0], engine=None)
