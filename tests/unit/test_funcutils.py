...
class FuncutilsTestCase(utils.BaseTestCase):
    ...
    def _old_wrapper(self, function):
    ...
    def test_old_wrapped_X(self):
        ...
        for _i in range(0, 10):
            wrapped = self._old_wrapper(wrapped)
    ...
    def _new_wrapper(self, function):

        @funcutils.wraps(function)
        def decorated_function(self, *args, **kwargs):
            function(self, *args, **kwargs)

        return decorated_function

    def test_new_wrapped_X(self):
        ...
        # Wrap it many times and ensure that its still the right one.
        for _i in range(0, 10):
            wrapped = self._new_wrapper(wrapped)
            self.assertTrue(hasattr(wrapped, '__wrapped__'))
            self.assertEquals(old_wrapped, wrapped.__wrapped__)
