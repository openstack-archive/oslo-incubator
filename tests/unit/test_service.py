...
class ServiceRestartTest(ServiceTestBase):
    ...
    def _spawn_service(self):
        ...
        if pid == 0:
            try:
                ...
                launcher.wait(ready_event=ready_event)
            except SystemExit as exc:
                status = exc.code
            os._exit(status)
        self.pid = pid
    ...
    def test_service_restart(self):
        ...
        timeout = 5
        self.assertTrue(ready.wait(timeout), 'Service never became ready')
        ready.clear()

        os.kill(self.pid, signal.SIGHUP)
        self.assertTrue(ready.wait(timeout), 'Service never back after SIGHUP')
