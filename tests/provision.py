...
def _execute_n_close(engine, sql, driver):
    """Initialize connection, execute sql query and close it."""
    conn = engine.connect()
    driver == 'postgresql' and conn.connection.set_isolation_level(0)
    for s in sql:
        conn.execute(s)
...
def create_database(engine):
    ...
    auth = dict(_zip_random(('name', 'user', 'password')))
    ...
    _execute_n_close(engine, sql, driver)
...
def drop_database(engine, current_uri):
    ...
    _execute_n_close(admin_engine, sql, driver)
